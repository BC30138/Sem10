...
Возвращает сигнал (не убивает процесс, как казалось). 

У Вадика есть классный список сигналов (полагаю можно загуглить и найти такой же).

Существуют сигналы типа T (terminate) - которые прерывают процессы. 

**SIGPIPE(posix)** - если этот сигнал получается, если в конструкции <команда1> | <команда2> одна из команд перестала отправлять данные и является типа T.

**SIGINT(posix)** (нажатие ctrl+c) - сигнал типа T, но некоторые процессы (например, вим) могут перехватывать сигнал.

```bash
$ kill -1 pipapache 
```

этот механизм провоцирует перечитывание конфига, либо остановку процесса, если процесс не обрабатывает его. (через него реализован reload).

**SIGSEGV(posix)** - сигнал типа C (core dump), отправляет системе информацию, что программа вызвала segmentation fault. Изначально core-файлы не создаются после завершение, то есть программа информирует о произошедшем систему, но не создает лог. это происходит потому что в настройках стоит размер core-файла 0. это можно посмотреть/изменить: 
...
Есть хорошая команда для остановки процесса: 
```bash
kill -9 <процесс>
```

**SIGSTOP** - сигнал типа S (suspend). Усыпляет процесс. (ctrl+z)

```bash
$ jobs #посмотреть все спящие процессы
# выдал, что код спящего %1
$ %1 # пробудили процесс
```

Такой трюк с графическими приложениями лучше не делать.

**SIGTTIN SIGTTOUT** - для ввода/вывода терминала.

[вроде бы ссылка с сигналами](http://people.cs.pitt.edu/~alanjawi/cs449/code/shell/UnixSignals.htm)

но в целом это содержится здесь: 
...
Демон - фоновая служба (специально для автономной работы).

Принцип UNIX-way - комнада должна делать только одну вещь и делать ее хорошо. Это особенно важно для демонов.

Однако этот принцип иногда нарушается.

Базовая структура демона: 
- Демон ответвляется от родительского процесса (fork). !!!  
- Прямо в демоне изменяется файловая маска. umaks(0)
- Доступ к открытию любых журналов на запись. <syslog.h> openlog(,,), syslog(,, ...), closelog(void)
- Создание уникального ID сессии (SID). !!! setsid()
- Изменение текущего каталога на безопасное место (если удалим существующий каталог). Как правило гарантировано безопасное место "/" chdir
- Закрытие стандартных файловых дескрипторов. close(STDIN_FILENO), close(STDOUT_FILENO), close(STDERR_FILENO)
- Цикл выполнения демона неограничен.
  
далее на слайде у Вадика есть пример запуска демона на языке C++. Да и в целом на слайдах описаны каждый из выше представленных пунктов.

Можно кинуть в лог что-нибудь так:
```bash
$ logger "HELLO WORLD"
$ tail /var/log/messages # либо /var/log/syslog
```

напишем прогу на C++ для работы с сислогом:
...
int main() {
    ...
    openlog("My first program", 0, 0); 
    syslog(0, "Hello world"); // сверхэкстренный приоритет
...
int main() {
    ...
    openlog("My first program", 0, LOG_LOCAL0); 
    syslog(LOG_NOTICE, "Hello world"); // приоритет по-проще

    return 0;
}
```

Обработка сигналов: (void signal_handler(int sig)) 
